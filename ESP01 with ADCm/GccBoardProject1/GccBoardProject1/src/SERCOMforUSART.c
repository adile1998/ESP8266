...
void uartInit(uint32_t fbaud){
	...
	NVIC_EnableIRQ(SERCOM5_IRQn); // global interrupt has been enabled.
	...
	SERCOM5->USART.CTRLA.bit.MODE = 0x1; // for usart with internal clock
	SERCOM5->USART.CTRLA.bit.CMODE = 0; // for async communication
	SERCOM5->USART.CTRLA.bit.RXPO = 0x3; // for pad[3] for rx pin pa23 as pad[1]
	SERCOM5->USART.CTRLA.bit.TXPO = 0x1;
	SERCOM5->USART.CTRLB.bit.CHSIZE = 0x0; // for 8 bits
	SERCOM5->USART.CTRLA.bit.DORD = 1;// most significant bit first
	SERCOM5->USART.CTRLA.bit.FORM = 0x0; // only frame, so without party bit
	SERCOM5->USART.CTRLB.bit.SBMODE = 0; // for one stop bit sbmode is 0, 2 stop bit sbmod is 1
	SERCOM5->USART.BAUD.bit.BAUD = baudRate;
	SERCOM5->USART.CTRLB.bit.RXEN = 1; // receiver is enabled
	SERCOM5->USART.CTRLB.bit.TXEN = 1;
	SERCOM5->USART.INTENSET.bit.RXC = 1; // receive compleate interrupt is enabled
	SERCOM5->USART.INTENSET.bit.ERROR = 1;
...
void uartWriteforser5(uint8_t *veri, uint8_t karakterAdedi){
	...
	for(uint8_t a = 0; a < karakterAdedi; a++) {
		...
		SERCOM5->USART.DATA.bit.DATA = *(veri + a);
